...
function App() {
  ...
  const postNewUser = (newUser) => {
    ...
    if (newUser.userType === "renter") {
      axios
        .post(
          "https://usemytechstuff2.herokuapp.com/api/renters/auth/register",
          postUser
        )
        .then((res) => {
      ...
      axios
        .post(
          "https://usemytechstuff2.herokuapp.com/api/owners/auth/register",
          postUser
        )
        .then((res) => {
  ...
  useEffect(() => {
    SignupFormSchema.isValid(userValues).then((valid) => {
      setDisabled(!valid);
    });
  }, [userValues]);
...
>>>>>>> 78931804f7d0abe319fa204d4e5c12e0debe1e8c
  return (
    <div>
      <Route exact path="/">
        <nav>
